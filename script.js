...
async function getData() { //This retrives data from the WAQI API for Pakuranga.
    ...
    try {
        ...
        else if (document.getElementById('aqiGraph')) {
            parseExtended(data);
        }
        else {
            parseMinimal(data);
...
function parseData(data) {
    if (data.status === "ok") {
        ...
        <hr>
        <a href="./about.html"><h3>Learn more!</h3></a>
    `;
...
//This function is called for the about page. Only the status header and footer get updated.
function parseMinimal(data) {
    if (data.status === "ok") {
        const { aqi, dominentpol, iaqi, city } = data.data;
        const { t, h, p, pm10, } = iaqi;
        document.getElementById('status').innerHTML = `
        <h2><strong>Current Status:</strong> ${getAqiCategory(aqi)}</h2>
        <hr>
        <a href="./"><h3>Back to the homepage</h3></a>
        `;
        document.getElementById('footer').innerHTML = `
    <p><strong>Last updated at:</strong> ${Date()}. All Data is from the World Air Quality Index Project. </p>
    `;
